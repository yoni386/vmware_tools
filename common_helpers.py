...
def vc_si(args):
    ...
    try:
        ...
        logger.info('Connecting to VC. Error: {}'.format(e))
...
class VimObj:
    ...
    def cl_res(self, si, cl_name):
        """
        TODO check if cl ComputeResource need to be used
        :param si:
        :param cl_name:
        :return:
        """
...
class VirtualMachine(VimObj):
    ...
    def get_all_vms_on_cluster(self, si, cl_name):
        ...
        return [vm for vm in VirtualMachine().return_vms(si, vm_name='any')
                if vm.resourcePool is not None and vm.resourcePool.parent == Cluster().return_clusters(si, cl_name)]
